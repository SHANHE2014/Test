 ...
 * Bitmap处理Beta版
 * <p>
 * Created by internet on 16/11/29.
 * Description:
 */
...
public class BitmapUtils {
     ...
     */
    public static byte[] scale(byte[] src, int maxSize) {
        try {
            Bitmap tempBitmap = byteToBitmap(src);
            Bitmap descBitmap = scale(tempBitmap, maxSize);
     ...
     */
    public static Bitmap scale(Bitmap src, int maxSize) {
        if (src == null)
            return null;
     ...
     */
    public static Bitmap scale(Bitmap src, float r) {
        if (src == null)
            return null;
        Matrix matrix = new Matrix();
     ...
     * @param b       源位图
     * @param degrees 旋转角度
     * @return 旋转后的位图，null 如果原位图b为空或者degrees为空 如果旋转后的图片仍和源位图一致，或出现OOM，则返回源位图，
     */
